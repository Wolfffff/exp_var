...
residuals_df_long <- residuals_df %>% tibble::rownames_to_column(var = "Row") %>% pivot_longer(cols = starts_with("E"), names_to = "gene",values_to = "value")
...
library(ggridges)
library(ggplot2)
ggplot(residuals_df_long, aes(x = value, y = source, fill = source)) +
  geom_density_ridges() +
  theme_ridges() + 
