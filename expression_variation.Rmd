...
for (dset_name in names(results_list_rc3)) {
  ...
  mean_df = reduce(mean_df_list, inner_join, by = "Genes")
  colnames(mean_df)[-1] = names(results_list_rc3)
```
```{r}
library(reshape2)
library(ggplot2)

xymelt <- melt(variance_df, id.vars = "Genes")
library(RColorBrewer)
n <- ncol(variance_df)
qual_col_pals = brewer.pal.info[brewer.pal.info$category == 'qual',]
col_vector = unlist(mapply(brewer.pal, qual_col_pals$maxcolors, rownames(qual_col_pals)))

ggplot(xymelt, aes(x = value, colour = variable)) +
  geom_density(show_guide=FALSE) + 
  stat_density(geom="line",position="identity") +
  scale_colour_manual(values=col_vector) +
  theme_classic() +
  labs(y="Density", 
       x="Variance of residuals", 
       title="Density plot of post correction expression data",
       colour="Study") +
  xlim(0,2)
  
ggsave(width=10,height=5,dpi=300,"mean_dist.png")
...
variance_df_long <- melt(mean_var$var$merged, id.vars = "Genes", variable.name = "study", value.name = "variance")
top_genes = variance_df_long %>% group_by(study) %>% slice_max(order_by = variance, n = 100)
top_genes_df = as.data.frame(table(top_genes$Genes))
top_genes_df$Freq <- top_genes_df$Freq / length(unique(top_genes$study))
ggplot(top_genes_df,aes(x=Freq)) + geom_histogram()
ggsave(width=10,height=5,dpi=300,"top_overlap.png")

top_genes_df[order(top_genes_df$Freq, decreasing = TRUE)[1:10],]
