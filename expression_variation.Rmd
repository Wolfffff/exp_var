...
for (dset_name in names(exp_data)) {
  ...
  for (column in names(feature_vec)) {
    if (column %in% colnames(filtered_metadata)){
      ...
      control_names_tb = control_names[names(control_names_tb) %in% feature_vec[[column]]]
  ...
  print(paste0("Unfiltered count dimensions: ", dim(countdata.list$counts)[1], " x ", dim(countdata.list$counts)[2]))
  print(paste0("Unfiltered metadata dimensions: ", dim(countdata.list$samples)[1], " x ", dim(countdata.list$samples)[2]))

  n.cores <- 8
  cuts <- cut(seq_len(nrow(countdata.voom)), n.cores)
  ...
  list_batch_effects_removed <- foreach(x=levels(cuts), .combine='c', .multicombine=TRUE) %dopar% { 
    library(limma)
    effects_removed <- removeBatchEffect(countdata.voom[cuts == x,],covariates=design_with_pc1) 
    list(effects_removed)
  }

  countdata_resids_with_pc1 <- do.call(rbind, list_batch_effects_removed)
  ...
  plt <- plot_grid(nrow=1, scale = 0.9, pca_on_voom + ggtitle("Uncorrected"), pca_on_resids + ggtitle("Known effects"), pca_on_resids_with_pc1 + ggtitle("Known effects and PC1"))
  ggsave(plt,filename=paste0(plots_dir,dset_name,"_pca.jpg"), width = 18, height = 6
  , dpi = 300)
