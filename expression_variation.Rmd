...
top_5q = as.integer((length(unique(variance_df_long$Genes))/100) * 5)
top_genes = variance_df_long %>% group_by(study) %>% slice_max(order_by = variance, n = top_5q)
top_genes_df = as.data.frame(table(top_genes$Genes))
top_genes_df$Freq <- top_genes_df$Freq / length(unique(top_genes$study))
top_genes_df = top_genes_df[order(top_genes_df$Freq,decreasing = TRUE),]
```